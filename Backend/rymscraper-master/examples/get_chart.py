...
def main(givenURL):
    ...
    if not givenURL == "https://rateyourmusic.com/charts/top/album/1/":
        url = givenURL
    ...
    #print(df)
    return df
    #export_filename = "top5000albums"
    #logger.info("Exporting results to %s.", export_filename + ".csv")

    df.to_pickle("top5000albums.pkl")
...
if __name__ == "__main__":
    url = "https://rateyourmusic.com/charts/top/album/1/"  # Get first album dataframe
    df = main(url)

    urls = []

    baseAlbum = "https://rateyourmusic.com/charts/top/album/"
    baseEP = "https://rateyourmusic.com/charts/top/ep/all-time/exc:live,archival/"
    baseMixtape = "https://rateyourmusic.com/charts/top/mixtape/all-time/"
    baseDJMix = "https://rateyourmusic.com/charts/top/djmix/all-time/"

    # for i in range(2, 126):
    #     urls.append(baseAlbum + str(i) + "/")

    for i in range(1, 11):
        urls.append(baseEP + str(i) + "/")

    for i in range(1, 3):
        urls.append(baseMixtape + str(i) + "/")

    for i in range(1, 3):
        urls.append(baseDJMix + str(i) + "/")

    for url in urls:
        newdf = main(url)
        df = pd.concat([df, newdf], ignore_index=True)

    df.to_pickle("top5000records.pkl")
    df.to_csv("top5000albums.csv", sep=",", index=False)
